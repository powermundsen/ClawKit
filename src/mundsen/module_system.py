"""Explicit, disabled-by-default extension points for local Mundsen modules."""
...
ModuleFactory = Callable[
    [MundsenPaths, RuntimeSettings, InstanceSettings],
    MundsenModule,
]
...
class ModuleRegistry:
    ...
    def create(
        ...
        try:
            ...
            raise ConfigurationError(f"unknown Mundsen module: {name}") from None
...
def _local_health_factory(
    ...
    del runtime
    from mundsen.modules.local_health import LocalHealthModule
...
def builtin_module_registry() -> ModuleRegistry:
    ...
    from mundsen.modules.command_connector import connector_factory
...
def parse_enabled_modules(
    ...
    *,
    ...
    if any(not _MODULE_RE.fullmatch(name) for name in names):
        raise ConfigurationError("MUNDSEN_MODULES contains an invalid module name")
    if len(set(names)) != len(names):
        raise ConfigurationError("MUNDSEN_MODULES contains duplicates")
    catalog = registry or builtin_module_registry()
    unknown = sorted(set(names) - set(catalog.names))
    if unknown:
        raise ConfigurationError(f"unknown Mundsen modules: {', '.join(unknown)}")
    return names
...
class ModuleManager:
    ...
    def require(self, name: str) -> MundsenModule:
        for module in self.modules:
            if module.name == name:
                return module
        raise ConfigurationError(
            f"module {name} is disabled; add it to MUNDSEN_MODULES in runtime.env"
        )
