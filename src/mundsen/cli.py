...
from mundsen.config import ConfigurationError, load_runtime_settings
...
from mundsen.instance import load_instance
from mundsen.module_system import ModuleManager
from mundsen.modules.local_health import LocalHealthError, LocalHealthModule
from mundsen.onboarding import OnboardingState, OnboardingStore
from mundsen.paths import MundsenPaths, PathConfigurationError
...
from mundsen.service import ServiceError, ServiceManager
from mundsen.setup import interactive_setup
from mundsen.skills import SkillError
...
def _paths(raw: str | None) -> MundsenPaths:
    selected = raw or os.environ.get("MUNDSEN_HOME") or str(Path.home() / "Mundsen")
    return MundsenPaths.from_root(_absolute_root(selected))
...
def _cmd_setup(paths: MundsenPaths, args: argparse.Namespace) -> int:
    ...
    if (
        ...
        and _confirm(
            "Allow Mundsen to send the first Telegram message when the bridge starts?"
        )
    ):
    ...
    if service_started:
        ...
        print(
            f"Start the bridge with `mundsen run` or `mundsen service install`. "
...
def _cmd_onboarding(paths: MundsenPaths, args: argparse.Namespace) -> int:
...
def _cmd_auth(paths: MundsenPaths, args: argparse.Namespace) -> int:
    selected = (
...
def _cmd_run(paths: MundsenPaths) -> int:
...
def _cmd_service(paths: MundsenPaths, action: str) -> int:
    manager = ServiceManager(paths)
    if action == "install":
...
def _cmd_health(paths: MundsenPaths, args: argparse.Namespace) -> int:
...
def _restart_if_installed(paths: MundsenPaths) -> bool:
    manager = ServiceManager(paths)
    if manager.status().installed:
        status = manager.restart()
        if not status.active:
            raise ServiceError("Mundsen service did not become active")
        return True
    return False


def _require_healthy(paths: MundsenPaths, *, service: bool) -> None:
...
def _service_is_installed(paths: MundsenPaths) -> bool:
...
def _cmd_upgrade(paths: MundsenPaths, args: argparse.Namespace) -> int:
    ...
    print(f"Mundsen {installed} is active.")
    return 0


def _cmd_rollback(paths: MundsenPaths, args: argparse.Namespace) -> int:
    ...
    print(f"Mundsen {version} is active.")
    return 0


def _cmd_update(paths: MundsenPaths, args: argparse.Namespace) -> int:
    runtime = load_runtime_settings(paths)
    repository = (
        args.repository
        or runtime.get("MUNDSEN_UPDATE_REPOSITORY")
        or DEFAULT_REPOSITORY
    )
    token = runtime.get("MUNDSEN_GITHUB_TOKEN")
    ...
    if args.json:
        ...
        print(f"Mundsen {current}; latest {release.version}; {status}.")
        changed = ", ".join(manifest.modules_changed) or "core"
        print(
...
def _cmd_training(paths: MundsenPaths, args: argparse.Namespace) -> int:
    runtime = load_runtime_settings(paths)
...
def _cmd_logs(paths: MundsenPaths, args: argparse.Namespace) -> int:
...
def _cmd_uninstall(paths: MundsenPaths, args: argparse.Namespace) -> int:
    manager = ServiceManager(paths)
    manager.uninstall_registration()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mundsen")
    parser.add_argument(
        "--home",
        help="Mundsen root directory (defaults to MUNDSEN_HOME or ~/Mundsen)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "service-run":
            print("Mundsen stopped after a local configuration error.", file=sys.stderr)
        else:
            print(f"Mundsen: {exc}", file=sys.stderr)
        return 1
    return 2
