...
SERVICE_NAME = "ai.mundsen.bridge"
...
def render_launch_agent(paths: MundsenPaths) -> bytes:
    payload = {
        "Label": SERVICE_NAME,
        "ProgramArguments": [
            str(paths.bin_dir / "mundsen"),
            "service-run",
        ],
        ...
        "EnvironmentVariables": {"MUNDSEN_HOME": str(paths.home)},
    }
    return plistlib.dumps(payload, fmt=plistlib.FMT_XML, sort_keys=True)


def render_systemd_service(paths: MundsenPaths) -> str:
    command = _systemd_quote(str(paths.bin_dir / "mundsen"))
    ...
    return "\n".join(
        (
            "[Unit]",
            "Description=Mundsen Telegram bridge",
            ...
            f"Environment={_systemd_quote(f'MUNDSEN_HOME={paths.home}')}",
            "Restart=on-failure",
            "RestartSec=5",
...
class ServiceManager:
    ...
    def registration_file(self) -> Path:
        ...
        return self.user_home / ".config" / "systemd" / "user" / "mundsen.service"
    ...
    def install(self) -> ServiceStatus:
        ...
        if self.kind == "launchd":
            ...
            result = self._run(
                ["systemctl", "--user", "enable", "--now", "mundsen.service"]
            )
        if result.returncode != 0:
            raise ServiceError(f"{self.kind} could not start Mundsen")
        return self.status()
    ...
    def stop(self) -> None:
        if self.kind == "launchd":
            ...
            self._run(["systemctl", "--user", "stop", "mundsen.service"])

    def start(self) -> ServiceStatus:
        if not self.registration_file.exists():
            raise ServiceError("the Mundsen service is not installed")
        if self.kind == "launchd":
            result = self._run(
                [
                    "launchctl",
            ...
            if result.returncode != 0 and not self.status().active:
                raise ServiceError("launchd could not start Mundsen")
        else:
            result = self._run(
                ["systemctl", "--user", "start", "mundsen.service"]
            )
            if result.returncode != 0:
                raise ServiceError("systemd could not start Mundsen")
        return self.status()
    ...
    def restart(self) -> ServiceStatus:
        if self.kind == "launchd":
            ...
            result = self._run(
                ["systemctl", "--user", "restart", "mundsen.service"]
            )
            if result.returncode != 0:
                raise ServiceError("systemd could not restart Mundsen")
        return self.status()
    ...
    def status(self) -> ServiceStatus:
        ...
        if self.kind == "launchd":
            ...
            result = self._run(
                ["systemctl", "--user", "is-active", "--quiet", "mundsen.service"]
            )
    ...
    def uninstall_registration(self) -> None:
        ...
        if self.kind == "systemd":
            self._run(["systemctl", "--user", "disable", "mundsen.service"])
