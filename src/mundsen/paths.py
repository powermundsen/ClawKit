"""Central, injectable filesystem paths for Mundsen."""
...
class PathConfigurationError(ValueError):
    """Raised when a configured Mundsen path is unsafe or ambiguous."""
...
class MundsenPaths:
    ...
    @classmethod
    def from_root(cls, root: str | Path) -> "MundsenPaths":
        """Build the portable single-directory installer layout."""

        root_dir = _absolute_path(root, "MUNDSEN_HOME")
        return cls(
            home=root_dir,
        ...
        *,
        ...
        return cls(
            ...
            config_dir=_configured_path(
                env, "MUNDSEN_CONFIG_DIR", xdg_config / "mundsen"
            ),
            data_dir=_configured_path(
                env, "MUNDSEN_DATA_DIR", xdg_data / "mundsen"
            ),
            state_dir=_configured_path(
                env, "MUNDSEN_STATE_DIR", xdg_state / "mundsen"
            ),
            cache_dir=_configured_path(
                env, "MUNDSEN_CACHE_DIR", xdg_cache / "mundsen"
            ),
            instance_dir=_configured_path(
                env, "MUNDSEN_INSTANCE_DIR", home_dir / "MundsenInstance"
            ),
        )
...
def ensure_private_directories(paths: Iterable[Path]) -> None:
    ...
    exact directory list, so constructing :class:`MundsenPaths` stays
    side-effect free.
    """
