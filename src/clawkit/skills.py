...
_REGISTRY = ".clawkit-managed.json"
...
def _skill_sources(paths: ClawKitPaths) -> dict[str, Path]:
    roots = (
        paths.current_release / "src" / "clawkit" / "bundled_skills",
...
def sync_skill_discovery(paths: ClawKitPaths) -> SkillSyncResult:
...
def skill_discovery_is_current(paths: ClawKitPaths) -> bool:
    try:
        sources = _skill_sources(paths)
        for root in (
