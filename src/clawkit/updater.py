...
DEFAULT_REPOSITORY = "powermundsen/ClawKit"
...
def _headers(token: str, *, binary: bool = False) -> dict[str, str]:
    ...
    headers = {
        ...
        "User-Agent": "ClawKit-update-check",
...
def fetch_release_manifest(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="clawkit-manifest-") as temporary:
...
class UpdateNotificationEngine:
    ...
    def pending(self) -> list[UpdateNotification]:
        ...
        text = (
            f"ClawKit {release.version} is available. Modules: {modules}. "
            f"Personal-data impact: {manifest.personal_data_impact}. "
            f"Migrations: {migrations}. Run `clawkit update check` to review it. "
            "Installation still requires explicit approval."
            if english
            else f"ClawKit {release.version} er tilgjengelig. Moduler: {modules}. "
            f"Persondataeffekt: {manifest.personal_data_impact}. "
            f"Migreringer: {migrations}. Kjør `clawkit update check` for detaljer. "
