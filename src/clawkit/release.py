...
from clawkit.instance import SCHEMA_VERSION
...
RELEASE_METADATA_NAME = ".clawkit-release.json"
...
def protected_snapshot(paths: ClawKitPaths) -> dict[str, str]:
...
def _extract_archive(archive: Path, destination: Path) -> Path:
    ...
    if not (release_root / "src" / "clawkit" / "__init__.py").is_file():
        raise ReleaseError("release archive does not contain ClawKit")
    return release_root


def _atomic_current(paths: ClawKitPaths, target: Path) -> None:
    if (
...
def active_version(paths: ClawKitPaths) -> str:
...
def activate_release(paths: ClawKitPaths, version: str) -> str:
    if not _VERSION_RE.fullmatch(version):
        raise ReleaseError("invalid release version")
...
def install_release(paths: ClawKitPaths, manifest_path: str | Path) -> str:
    ...
    if manifest.migrations:
        raise ReleaseError(
            "this ClawKit version does not support release migrations"
        )
...
def rollback_release(paths: ClawKitPaths, version: str = "") -> str:
